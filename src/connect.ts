...
import { searchRoleProfiles, clickConnectOnSearch, clickFollowOnSearch, handleConnectPopup, goToNextPage } from "./search.ts";
...
export async function processTechRoleConnections(
  ...
  for (const roleConfig of ROLE_CONFIGS) {
    ...
    if (processedThisRole === 0) {
      ...
      console.log(`   ✓ ${processedThisRole}/${targetPerRole} ${roleConfig.displayName} — ${scannedThisRole} profil di-scan`);
    }
  }
